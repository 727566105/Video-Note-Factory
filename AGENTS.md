...
- **合集总结与单条笔记完全独立**：`SummarySettings` 组件按 `variant` 区分——`note`（笔记详情/快捷添加，全量：风格/语言/视频理解/格式/备注）vs `collection`（合集，**只显示「总结模式 + 备注」**）。**总结模式下拉（`summaryModes`）只显示 overview/comparison/timeline/trajectory 4 项**——**mindmap（思维导图）不属于「总结模式」选择**：它是页面「归纳总结」区的独立快捷按钮（`CollectionDetail` 的 `handleGenerate('mindmap')`），因此 `summaryModes` 与 `CollectionDetail` 的模式同步白名单都不含 mindmap（合集当前总结是 mindmap 时，设置弹窗回退显示全局/默认模式，重新总结不会再用 mindmap——想生成思维导图只能点页面按钮）。后端 `collection.py` 的 `mode_prompts` 仍保留 5 套（思维导图按钮要用）。新增模式枚举需同步：`SummarySettings` 的 `summaryModes` 数组 + `CollectionDetail` 的 mode 白名单 + `collection.py` 的 `mode_prompts`。
...
- **⚠️ 合集总结素材转写兜底（笔记缺失时）**：`_generate_collection_summary_inner`（`services/collection.py`）素材读取顺序——笔记 `note_{user_id}.json` 的 `markdown` 优先（>2000 字截断）；`markdown` 缺失/为空/纯空白（`strip()` 判断）→ 读 `transcript.json` 的 `full_text` 全文（**不截断**；`full_text` 空/缺失/非 str 时拼接 `segments[].text`，segments 用 `isinstance(seg, dict)` 守卫）。转写条目 header 尾部追加 `[转写原文]` 标注（两种模式都加），提示 LLM 这是未提炼素材。笔记与转写均缺失 → warning + 跳过该条目。改此逻辑看 `tests/test_collection_profile_summary.py` 的转写兜底系列测试。
- **⚠️ 合集分批总结是动态分批（防转写全文撑爆上下文）**：`_batch_summarize` 固定 3 篇一批已改为**按累计字符数动态分批**——每批 ≤ `MAX_BATCH_CHARS`(=8000)，整篇不跨批；单篇 > `LONG_ENTRY_CHARS`(=8000) 先经 `_summarize_long_entry` 单独压缩为 ≤ `LONG_ENTRY_SUMMARY_LEN`(=1000) 字摘要（`GPTSource(title="long-entry")`，压缩失败降级截断 8000 参与批次）。12000 字触发阈值不变。常量在 `collection.py` 模块区。
- **⚠️ generate_summary 后端 single-flight（同合集并发只执行一次 LLM）**：`generate_collection_summary`（`services/collection.py`）是编排层——模块级 `_generation_inflight`（`threading.Event` + slot dict，`_generation_guard` 锁保护）确保同一 `collection_id` 并发生成时只有首个请求调用 LLM，等待者 `event.wait(600)` 后复用其结果（成功返回同一结果、失败返回 None、未预期异常透传），防重复计费与结果互相覆盖。真实实现逻辑在 `_generate_collection_summary_inner`。测试 `test_collection_blocking.py::test_generate_summary_single_flight_runs_llm_once`。改并发逻辑务必保留该防线。
...
- **Cookie 功能是管理员专属（多层防御，勿放开）**：后端 `update_downloader_cookie` / `test_downloader_cookie` / `get_downloader_cookie/{platform}`（`config.py`）均 `require_admin`（未登录 401、普通用户 403）；插件 `popup.js` 多层防御——① `applyCookiePermission()` 隐藏非管理员的 Cookie Tab（**cookie tab 在 `popup.html` 内联 `style="display:none"` 默认隐藏**，防 getAuth 异步完成前闪烁；admin 时 `style.display=''` 清除内联隐藏）② localStorage 恢复 Tab 需 `authRole==='admin'` ③ `onPush()` 运行时独立 `getAuth()` 二次校验 ④ **`chrome.storage.onChanged` 监听 `authRole` 变化重跑 `applyCookiePermission`**（popup 保持打开期间在 options 登出/换账号也能实时隔离）⑤ `onGetCookie`/`onCopy`/`onCopyNetscape` 统一走 `requireAdminForCookie()` 静默守卫（fail-closed，与 onPush 一致，不弹提示不泄露功能）。`authRole` 缺失按非管理员 fail-closed。**UI 不显示"推 Cookie 需管理员"类提示文案**（用户明确要求移除，普通用户直接看不到 Cookie 功能即可）；**登录前文案也不得提及 Cookie**（登录遮罩只说"提交笔记任务"，authGate 阶段无法区分角色，提及即向普通用户泄露功能存在）。`update_downloader_cookie` 有审计日志（记录 username/id/platform）。权限矩阵测试见 `tests/test_cookie_endpoint_auth.py`（401/403/200 + 输入边界）。
